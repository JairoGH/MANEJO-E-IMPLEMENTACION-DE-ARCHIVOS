package Reportes

import (
	"backend/Entornos"
	"backend/Particiones"
	"backend/Usuarios"
	"backend/Utils"
	"fmt"
	"os"
	"path/filepath"
	"strings"
)

// GenerarReporteFile lee el contenido exacto de un archivo del sistema de archivos simulado
// y lo guarda en un archivo de texto en la ruta especificada.
func GenerarReporteFile(pathFileLs, outputPath, id string) string {
	var output strings.Builder // Usar strings.Builder para el log de salida

	// --- 1. Validaciones Iniciales ---
	if !Usuarios.IsUserLoggedIn() {
		output.WriteString("Error: No hay una sesión activa. Use 'login' primero.\n")
		return output.String()
	}
	if strings.TrimSpace(outputPath) == "" {
		output.WriteString("Error: El parámetro de salida -path no puede estar vacío.\n")
		return output.String()
	}
	if strings.TrimSpace(id) == "" {
		output.WriteString("Error: El parámetro -id no puede estar vacío.\n")
		return output.String()
	}
	if strings.TrimSpace(pathFileLs) == "" {
		output.WriteString("Error: Debe especificar la ruta del archivo a leer con -path_file_ls.\n")
		return output.String()
	}

	// --- 2. Obtener Partición y Superbloque ---
	mountedPartition, found := Entornos.GetMountedPartitionByID(id)
	if !found {
		output.WriteString(fmt.Sprintf("Error: No se encontró la partición con ID %s montada\n", id))
		return output.String()
	}

	file, err := Utils.OpenFile(mountedPartition.MountPath)
	if err != nil {
		output.WriteString(fmt.Sprintf("Error: No se pudo abrir el disco: %v\n", err))
		return output.String()
	}
	defer file.Close()

	var superblock Particiones.SuperBlock
	if err := Utils.ReadFile(file, &superblock, int64(mountedPartition.MountStart)); err != nil {
		output.WriteString(fmt.Sprintf("Error: No se pudo leer el superbloque: %v\n", err))
		return output.String()
	}

	// --- 3. Encontrar y Validar el Inodo del Archivo ---
	inodeIndex, _ := Usuarios.InitSearch(pathFileLs, file, superblock)
	if inodeIndex == -1 {
		output.WriteString(fmt.Sprintf("Error: Archivo '%s' no encontrado en el sistema de archivos.\n", pathFileLs))
		return output.String()
	}

	var fileInode Particiones.Inode
	inodePos := superblock.S_inode_start + inodeIndex*superblock.S_inode_size
	if err := Utils.ReadFile(file, &fileInode, int64(inodePos)); err != nil {
		output.WriteString(fmt.Sprintf("Error al leer el inodo del archivo: %v\n", err))
		return output.String()
	}

	// Verificar que sea un archivo regular (tipo '1')
	if fileInode.I_type[0] != '1' {
		output.WriteString(fmt.Sprintf("Error: La ruta '%s' no corresponde a un archivo.\n", pathFileLs))
		return output.String()
	}

	// --- 4. Leer el Contenido del Archivo (sin limpiarlo) ---
	// La función GetInodeFileData ya se encarga de concatenar los bloques y truncar al tamaño correcto.
	content, log := Usuarios.GetInodeFileData(fileInode, file, superblock)
	output.WriteString(log + "\n") // Capturar el log de la lectura de bloques

	// --- 5. Guardar el Contenido Exacto en el Archivo de Salida ---
	// Asegurarse de que el directorio de salida exista
	if err := os.MkdirAll(filepath.Dir(outputPath), 0755); err != nil {
		output.WriteString(fmt.Sprintf("Error al crear directorios de salida: %v\n", err))
		return output.String()
	}
	// Escribir el contenido crudo al archivo
	if err := os.WriteFile(outputPath, []byte(content), 0644); err != nil {
		output.WriteString(fmt.Sprintf("Error al guardar el archivo de reporte: %v\n", err))
		return output.String()
	}

	output.WriteString(fmt.Sprintf("Contenido del archivo '%s' guardado localmente en: %s\n", pathFileLs, outputPath))

	// --- 6. Subir a S3 ---
	bucketName := "proyecto2-front"
	// Usamos outputPath, que es el archivo .txt (o la extensión que sea) que se guardó localmente
	reportS3Key := "reports/" + filepath.Base(outputPath)

	publicURL, errS3 := Utils.UploadS3(bucketName, outputPath, reportS3Key)
	if errS3 != nil {
		output.WriteString(fmt.Sprintf("Error al subir el archivo a S3: %v\n", errS3))
	} else {
		output.WriteString(fmt.Sprintf("Archivo subido a S3 exitosamente.\n"))
		output.WriteString(fmt.Sprintf("URL Pública: %s\n", publicURL))
	}

	return output.String()
}
