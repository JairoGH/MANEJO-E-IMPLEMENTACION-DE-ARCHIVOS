...
func GenerarReporteDisk(diskPath, reportPath string) string {
	...
	output.WriteString(fmt.Sprintf("PDF generado localmente en: %s\n", outputPdf))

	// Declaracion del nombre del S3
	bucketName := "proyecto2-front"

	reportS3Key := "reports/" + filepath.Base(outputPdf)

	publicURL, err := Utils.UploadS3(bucketName, outputPdf, reportS3Key)
	if err != nil {
		output.WriteString(fmt.Sprintf("Error al subir el PDF a S3: %v\n", err))
	} else {
		output.WriteString(fmt.Sprintf("PDF subido a S3 exitosamente.\n"))
...
func GenerateDiskReport(mbr Particiones.MBR, ebrs []Particiones.EBR, outputPath string, file *os.File, totalDiskSize int32, diskLabel string) error {
	...
	for i := 0; i < 4; i++ {
		...
		if string(partition.Part_Type[:]) == "p" { // Primaria
			...
			graphContent.WriteString("\t\t\t\t\t\t<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"6\">\n")
			graphContent.WriteString(fmt.Sprintf("\t\t\t\t\t\t\t<TR><TD COLSPAN=\"100\" BGCOLOR=\"#f39c12\"><FONT COLOR=\"white\" POINT-SIZE=\"14\"><B>Extendida</B></FONT><BR/><FONT POINT-SIZE=\"12\">%d bytes</FONT><BR/><FONT POINT-SIZE=\"12\">%.2f%%</FONT></TD></TR>\n", extendedSpace, percentage))
			graphContent.WriteString("\t\t\t\t\t\t\t<TR>\n")
