...
func GenerarReporteMBR(diskPath, reportPath string) string {
	...
	if err := Utils.ReadFile(file, &TempMBR, 0); err != nil {
		output.WriteString("Error: No se pudo leer el MBR desde el archivo\n")
		return output.String()
	}
	...
	if err := GenerateMBRReport(TempMBR, ebrs, outImgPath, file); err != nil {
		output.WriteString(fmt.Sprintf("Error al generar el reporte MBR: %v\n", err))
		return output.String()
	}
	...
	// outImgPath es la imagen generada (ej: .jpg, .png)
	reportS3Key := "reports/" + filepath.Base(outImgPath)

	publicURL, errS3 := Utils.UploadS3(bucketName, outImgPath, reportS3Key)
	if errS3 != nil {
		output.WriteString(fmt.Sprintf("Error al subir la imagen a S3: %v\n", errS3))
	} else {
		output.WriteString(fmt.Sprintf("Imagen subida a S3 exitosamente.\n"))
