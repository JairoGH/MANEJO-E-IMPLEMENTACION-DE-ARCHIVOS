...
func GenerarReporteLS(pathFileLs string, outputPath string, id string) string {
	...
	if !Usuarios.IsUserLoggedIn() {
		out.WriteString("Error: No hay una sesión activa. Use 'login' primero.\n")
		return out.String()
	}

	mp, ok := Entornos.GetMountedPartitionByID(id)
	if !ok {
		out.WriteString(fmt.Sprintf("Error: No se encontró la partición con ID %s montada\n", id))
		return out.String()
	}

	dot, err := generateLSDotContent(*mp, pathFileLs)
	if err != nil {
		out.WriteString(fmt.Sprintf("Error al generar contenido LS: %v\n", err))
		return out.String()
	}

	if err := os.MkdirAll(filepath.Dir(outputPath), 0755); err != nil {
		out.WriteString(fmt.Sprintf("Error al crear directorios de salida: %v\n", err))
		return out.String()
	}

	dotPath := strings.TrimSuffix(outputPath, filepath.Ext(outputPath)) + ".dot"
	if err := os.WriteFile(dotPath, []byte(dot), 0644); err != nil {
		out.WriteString(fmt.Sprintf("Error al escribir archivo DOT: %v\n", err))
		return out.String()
	}

	// Verificar 'dot' disponible
	if _, err := exec.LookPath("dot"); err != nil {
		out.WriteString("Advertencia: Graphviz no está instalado o 'dot' no está en PATH.\n")
		out.WriteString(fmt.Sprintf("Dejé el .dot en: %s\n", dotPath))
		out.WriteString(fmt.Sprintf("Para generar JPG: dot -Tjpg %s -o %s\n", dotPath, outputPath))
		return out.String()
	}

	cmd := exec.Command("dot", "-Tjpg", dotPath, "-o", outputPath)
	if gvOut, err := cmd.CombinedOutput(); err != nil {
		out.WriteString(fmt.Sprintf("Error al generar imagen con Graphviz: %v\nSalida: %s\n", err, string(gvOut)))
		return out.String()
	}

	out.WriteString("Reporte LS generado localmente:\n")
	...
	if errS3 != nil {
		out.WriteString(fmt.Sprintf("Error al subir el JPG a S3: %v\n", errS3))
	} else {
		out.WriteString(fmt.Sprintf("JPG subido a S3 exitosamente.\n"))
		out.WriteString(fmt.Sprintf("URL Pública: %s\n", publicURL))
	}
